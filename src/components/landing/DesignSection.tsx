          ...
          <AnimatedTransition show={!animating} animation="fade" duration={500}>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {currentTemplates.map((template, idx) => (
                ...
                >
                  <div className="relative h-40 bg-gradient-to-br from-primary/5 to-accent/20 p-6 flex items-center justify-center">
                    <div className="absolute inset-0 bg-primary/5 group-hover:bg-primary/10 transition-colors duration-300" />
                    <span className="font-medium text-lg text-center text-foreground z-10 group-hover:scale-105 transition-transform duration-300">
                      {template}
                    </span>
                  </div>
                </Card>
